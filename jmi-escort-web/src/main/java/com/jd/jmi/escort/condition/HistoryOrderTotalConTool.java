 ...
 */
public class HistoryOrderTotalConTool extends  AbstractConditionTool implements ConditionToolInterface{

    private static final Logger logger = LoggerFactory.getLogger(HistoryOrderTotalConTool.class);

    @Override
    public ConditionResult getValidateValue(String operate,String value) {
        ConditionResult result = validateOperator(operate,ConditionType.RISK_HISTORY_ORDER_TOTAL);
        if(result !=null){
            return result;
        }
        result = new ConditionResult(false,ConditionType.RISK_HISTORY_ORDER_TOTAL.getName()+"必须为大于0的整数",value);
        try {
    ...
    public String translate(String operate, String value) {
        try{
            return ConditionType.RISK_HISTORY_ORDER_TOTAL.getName()+ getOperateValue(operate)+value;
        }catch (Exception e){
