 ...
 */
public class OrderAmountConTool extends  AbstractConditionTool implements ConditionToolInterface{

    private static final Logger logger = LoggerFactory.getLogger(OrderAmountConTool.class);

    @Override
    public ConditionResult getValidateValue(String operate,String value) {
        ConditionResult result = validateOperator(operate,ConditionType.RISK_ORDER_AMOUNT);
        if(result !=null){
            return result;
        }
        result = new ConditionResult(false,ConditionType.RISK_ORDER_AMOUNT.getName()+"必须为大于0的整数",value);
        try {
    ...
    public String translate(String operate, String value) {
        try{
            return ConditionType.RISK_ORDER_AMOUNT.getName()+ getOperateValue(operate)+value;
        }catch (Exception e){
